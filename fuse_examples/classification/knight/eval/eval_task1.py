...
### A.G, TODO: this is a temporary patch for running evaluation on Task 1 only
### currently I don't see how it's possible with the original code, as 
### there is no way to pass arguments to pre_collect_process_func, other than sample_dict
### we need to fix this in eval.py
import pathlib
import sys
import os
...
def post_processing1(sample_dict: dict, task1: bool = True, task2: bool = False) -> dict:
    """
...
def eval(task1_prediction_filename: str, task2_prediction_filename: str, target_filename: str, output_dir: str, case_ids_source: Optional[Union[str, List[str]]] = "target") -> Tuple[OrderedDict, str]:
    ...
    if task1:
        # metrics to evaluate
        metrics.update({
            "task1_auc": CI(MetricAUCROC(pred='task1_pred.array', target='target.Task1-target', class_names=TASK1_CLASS_NAMES, pre_collect_process_func=post_processing1), stratum="target.Task1-target"),
            "task1_roc_curve": MetricROCCurve(pred='task1_pred.array', target='target.Task1-target', class_names=[None, ""], pre_collect_process_func=post_processing1, output_filename=os.path.join(output_dir, "task1_roc.png")),
        })
        # read files
    ...
    if task2:
        # metrics to evaluate
        metrics.update({
            "task2_auc": CI(MetricAUCROC(pred='task2_pred.array', target='target.Task2-target', class_names=TASK2_CLASS_NAMES, pre_collect_process_func=post_processing1), stratum="target.Task2-target"),
            "task2_roc_curve": MetricROCCurve(pred='task2_pred.array', target='target.Task2-target', class_names=TASK2_CLASS_NAMES, output_filename=os.path.join(output_dir, "task2_roc.png"), pre_collect_process_func=post_processing1),
        })
        # read files
