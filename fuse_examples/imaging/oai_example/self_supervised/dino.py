...
class DINO(pl.LightningModule):
    ...
    def training_step(self, batch_dict, batch_idx):
        ...
        momentum = cosine_schedule(self.current_epoch, self.cfg.n_epochs, self.cfg.momentum_teacher, 1)
        ...
        teacher_out, t_embs = zip(*[self.forward_teacher(view) for view in global_views])
        # if (self.current_epoch+1) % 8 != 0:
        student_out = [self.forward(view) for view in views]
        dino_loss = self.criterion(teacher_out, student_out, epoch=self.current_epoch)
        if self.cfg.clearml:
            self.cfg.logger.report_scalar("dino_loss", "dino_loss", iteration=self.current_step, value=dino_loss.item())
    ...
    def validation_step(self, batch_dict, batch_idx):
        ...
        dino_loss = self.criterion(teacher_out, student_out, epoch=self.current_epoch)
        if self.cfg.clearml:
            self.cfg.logger.report_scalar("dino_loss", "val_dino_loss", iteration=self.current_step, value=dino_loss.item())
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = {"scheduler" : CosineAnnealingLR(optimizer=optimizer, T_max=self.cfg.n_epochs)}
...
def main(cfg: DictConfig):
    ...
    if cfg.clearml:
        ...
        clearml_task_id = glob(logging_path + "/*.cmlid")[0].split("/")[-1].split(".")[0]
        task = Task.init(project_name='OAI/Dino', task_name=cfg.experiment, reuse_last_task_id=clearml_task_id,
    ...
    # train
    pl_trainer.fit(pl_module, train_dl, val_dl,
                    ckpt_path=ckpt_path
                   )
