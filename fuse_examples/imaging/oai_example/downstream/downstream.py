...
def main(cfg: DictConfig):
    ...
    ##############################################################################
    columns_to_extract = (
        ["accession_number", "path"] + cfg.reg_targets + cfg.cls_targets
    )
    train_dls = []
    val_dls = []
    ...
    if len(cls_targets) > 0:
        ...
        cls_targets = {
            name: {
                "classes": list(cls_train_df[name].unique()),
                **dict(cls_train_df[name].value_counts(normalize=True)),
            }
            for name in cls_targets
        }
        print(cls_targets)
        for cls_target, stats in cls_targets.items():
            cls_train_df[cls_target] = cls_train_df[cls_target].apply(
                lambda x: stats["classes"].index(x)
            )
            cls_val_df[cls_target] = cls_val_df[cls_target].apply(
                lambda x: stats["classes"].index(x)
            )

        print(f"cls train samples = {len(cls_train_df)}")
        print(f"cls val samples = {len(cls_val_df)}")
        cls_train_ds = OAI.dataset(
            cls_train_df,
            for_classification=True,
            validation=True,
            columns_to_extract=columns_to_extract,
    ...
    ##############################################################################
    losses = {
        f"loss_{target}": LossDefault(
            pred=f"model.output.head_{target}",
            target=target,
            callable=nn.MSELoss(),
            weight=1.0,
        )
    ...
    val_metrics = {
        f"mae_{target}": MetricDefault(
            metric_func=my_mae_func, pred=f"model.output.head_{target}", target=target
        )
        for target in reg_targets
    }
    if len(cls_targets) > 0:
