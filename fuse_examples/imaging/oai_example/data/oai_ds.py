...
from data.data_ops import OpLoadData, OpNormalizeMRI, OpResize3D, OpVolumentation, OpMask3D, OpDinoCrops, OpRandomCrop, OpRandomFlip
...
class OAI:
    ...
    @staticmethod
    def static_pipeline(df: pd.DataFrame, columns_to_extract, resize_to = None) -> PipelineDefault:
        # to_extract = 
        static_pipeline = PipelineDefault("static", [
         # decoding sample ID
            (OpReadDataframe(df, key_column="accession_number", columns_to_extract=columns_to_extract),dict()),
    ...
    @staticmethod
    def dynamic_pipeline(n_crops = 2, mae_cfg = None,
                         for_classification: bool= True, validation: bool = False, resize_to: Sequence = [40,224,224]):
        """
        ...
        if for_classification:
            if validation:
                ...
                (OpRepeat(OpLambda(partial(torch.unsqueeze, dim=0)), repeat_for), dict()),
            ])
            else:
                dynamic_pipeline.extend([
            ...
            if validation:
                dynamic_pipeline.extend([
                (OpResize3D(), dict(key="img", shape=resize_to)),
                (OpMask3D(mask_percentage=mae_cfg["mask_percentage"],cuboid_size=mae_cfg["cuboid_size"]),dict(key="img")),
                (OpRepeat(OpToTensor(), repeat_for), dict(dtype=torch.float32)),
                (OpRepeat(OpLambda(partial(torch.unsqueeze, dim=0)), repeat_for), dict()),
            ])
            else:
                dynamic_pipeline.extend([
                    (OpVolumentation(Compose([
            ...
            dynamic_pipeline.extend([
                ...
                (OpRepeat(OpRandomCrop(), repeat_for[:2]), dict(scale=(0.8,1.0), on_depth=False)),
                (OpRepeat(OpRandomCrop(), repeat_for[2:]), dict(scale=(0.1,0.5), on_depth=False)),
                (OpRepeat(OpVolumentation(Compose([
                ...
                # Global Gaussian 1
                (OpVolumentation(Compose([
                    GaussianNoise(var_limit=(0.001,0.1),p=0.6)
                    ])), dict(key="crop_0")),
                ...
                (OpRepeat(OpResize3D(), repeat_for[:2]), dict(shape=[40,224, 224])),
                (OpRepeat(OpResize3D(), repeat_for[2:]), dict(shape=[40, 128, 128])),
                (OpRepeat(OpToTensor(), repeat_for), dict(dtype=torch.float32)),
                (OpRepeat(OpLambda(partial(torch.unsqueeze, dim=0)), repeat_for), dict()),
    ...
    def dataset(
        ...
        dynamic_pipeline = OAI.dynamic_pipeline(mae_cfg=mae_cfg,
         for_classification=for_classification, validation=validation, n_crops=n_crops, resize_to=resize_to)


        if cache_dir != None:                        
            cacher = SamplesCacher(f'oai', 
                static_pipeline,
