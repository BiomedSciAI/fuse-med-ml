...
def validate_naming_conventions(naming_conventions):
    ...
    assert (
        "svc_date_key" in naming_conventions
    )  # not strictly necessary - we can do without this column, if admission date column (adm_date_key) exists
...
class TimeFilter:
    ...
    ref_date_column_name = (
        None  # name of a column containing a date to be used as a reference for filtering (e.g. index date)
    )
    ...
    def apply_filter(self, df):
        ...
        if self.stop_days_before is not None:
            if isinstance(df.iloc[0][self.ref_date_column_name], str):
                tmp = df[self.ref_date_column_name].apply(
                    lambda x: datetime.strptime(x, self.date_format_string).date()
                )
                df["days_before_index"] = tmp.apply(
                    lambda x: x.year * 365 + x.month * 31 + x.day * 1 if (pd.notnull(x)) else x
                ) - df[self.date_column_name].apply(
                    lambda x: x.year * 365 + x.month * 31 + x.day * 1 if (pd.notnull(x)) else x
                )
            else:
                df["days_before_index"] = df[self.ref_date_column_name].apply(
                    lambda x: x.year * 365 + x.month * 31 + x.day * 1 if (pd.notnull(x)) else x
                ) - df[self.date_column_name].apply(
                    lambda x: x.year * 365 + x.month * 31 + x.day * 1 if (pd.notnull(x)) else x
                )
...
class SplitFilter:
    ...
    def apply_split(self, df):
        ...
        if self.split_method == "new":  # use tr_val_tst ratios to split the df:
            ...
            df[self.split_key] = np.random.choice(
                a=self.tr_val_tst_names, size=df.shape[0], replace=True, p=self.tr_val_tst_ratios
            )
...
class DataContainer:
    ...
    inddate_filter = {"train": None, "validation": None, "test": None}
...
class GTContainer(DataContainer):
    ...
    def add_event_df(
        self, df, event_names: list = None, event_naming_conventions: dict = None, same_visit_date_diff: int = 1
    ):
        """
        ...
        for ev_name in event_names:
            in_df[ev_name] = in_df.apply(
                lambda x: x[event_naming_conventions["event_date_key"]]
                if x[event_naming_conventions["event_type_key"]] == ev_name
                else None,
                axis=1,
            )
        ...
        aggr_dict = {}
        for ev_name in event_names:
            aggr_dict[ev_name] = lambda x: [
                l for l in x if pd.notna(l)
            ]  # TODO: check if we get double entries for a given event (resulting in date duplicates). This is improbable, and if it happens it won't cause a problem except for somewhat slowing overall calculations.
        ...
        if self.main_df == None:
            ...
            self.main_df = self.main_df.merge(
                in_df, how="outer", on=self.output_naming_conventions["patient_id_key"], suffixes=("", "_right")
            )
    ...
    def add_gt_values_to_external_df(self, ext_df):
        ...
        ext_df = ext_df.merge(
            self.main_df, how="left", on=self.output_naming_conventions["patient_id_key"], suffixes=("", "_right")
        )
        ...
        for ev_name in modes:
            ...
            ext_df[ev_name] = ext_df.apply(
                lambda x: [
                    (ev_date - x[self.output_naming_conventions["date_key"]]).days
                    for ev_date in x[ev_name]
                    if (ev_date - x[self.output_naming_conventions["date_key"]]).days > 0
                ]
                if isinstance(x[ev_name], list)
                else [],
                axis=1,
            )
        ...
        )  # This collects into list, and ignores NaN's.
...
class InputDataContainer(DataContainer):
    ...
    def ingest_df(
        ...
        for df in tqdm(input_dfs):
            ...
            df = df[
                df[diagnosis_col_new].notna()
            ]  # TODO: check if all these dropna's are legit (maybe export the dropna column definitions to the config)
            ...
            if age_month_key not in df:
                df[age_month_key] = df.apply(
                    lambda x: (x[date_key].year - int(x[date_birth_key])) * 12 + x[date_key].month, axis=1
                )  # TODO: This takes a LONG time to calculate. See if it can be made more efficient
            ...
            if age_key not in df:
                df[age_key] = df.apply(lambda x: (x[date_key].year - x[date_birth_key]), axis=1)
            ...
            agg_dict = {
                diagnosis_col_new: lambda x: (list(set([l for l in x if pd.notna(l)])) + [end_of_sentence_sep]),
    ...
    def apply_split(self, splitter):
        ...
        split_input_containers = {
            k: InputDataContainer(output_naming_conventions=self.output_naming_conventions, init_df=split_dfs[k])
            for k in split_dfs
        }
    ...
    def vectorize_inputs(self, output_naming_conventions, training_type, mode=None, min_visits=0):
        ...
        if mode == "all" or mode == "nolast" or mode == "last" or mode == "random":
            ...
            self.main_df[diagnosis_col_new] = self.main_df.apply(
                lambda x: x[diagnosis_col_new][: x["visit_len_cum"]], axis=1
            )
...
class DBExtractor:
    ...
    split_data_containers = (
        None  # dictionary of {set_name: DataContainer}, where set_name is one of ('train', 'validation', 'test')
    )

    def __init__(
        self,
        output_naming_conventions: dict,
        ...
        """
        self.gt_containers = {
            gt_target_key: GTContainer(
                output_naming_conventions=output_naming_conventions,
                modes=gt_modes[gt_target_key],
                target_key=gt_target_key,
            )
            for gt_target_key in gt_target_keys
        }
    ...
    def ingest_sql(
        ...
        input_naming_conventions,
        event_naming_conventions,
        output_naming_conventions,
        str_external_gt=None,
        gt_target_key=None,
        str_data=None,
        same_visit_date_diff=1,
        data_source="sql_db",
        ...
        if data_source == "csv":
            if str_data != None:
                ...
                parse_dates = [
                    input_naming_conventions["adm_date_key"],
                    input_naming_conventions["svc_date_key"],
                    input_naming_conventions["index_date_key"],
                ]
                data_df = pd.read_csv(str_data, sep=",", parse_dates=parse_dates)
                data_df = self.translate_df(data_df, input_naming_conventions)
                # Vadim:
                self.data_container_inst.ingest_df(
                    data_df,
                    task=self.task,
                    fields_to_keep=self.columns_to_keep,
            ...
            if str_external_gt != None:
                ...
                self.gt_containers[gt_target_key].add_event_df(
                    df=gt_df,
                    event_names=None,
    ...
    def vectorize_inputs(
        self, output_naming_conventions, training_type, tr_set: str, keep_trajectories: str = None, min_visits: int = 0
    ):
        """
        ...
        if tr_set != "all":
            assert tr_set in self.split_data_containers
            self.split_data_containers[tr_set].vectorize_inputs(
                output_naming_conventions, training_type, mode, min_visits
            )
        else:
    ...
    def output_to_files(self, file_config: dict, tr_val_tst_names: list, tr_val_test_dirs: list, format: str = "BEHRT"):
        ...
        for gt_event_key in self.gt_containers:
            vocab_gt_out_path = get_vocab_path(
                root_vocab_path=file_config["gt_vocab"], event_type_identifier=gt_event_key
            )
...
def transform_data(config_file: str):
    ...
    limit_visits = data_config[
        "limit_visits"
    ]  # 50000000 #10000 #None #don't take less than 10k, otherwise you'll get only a single visit per patient, which is not enough in case of next visit prediction
    x_sub = "x_data"
    y_sub = "y_data"
    ...
    if do_write_files:
        ...
        naming_conventions_event = {
            output_naming_conventions["event_key"]: {  # TODO: move this to config
                "patient_id_key": "ENROLID",
        ...
        gt_modes = {
            output_naming_conventions["event_key"]: {
                "all": {"future_timeframe": data_config["event_prediction_window_days"]}
            },  #'within_90_days',
            output_naming_conventions["treatment_event_key"]: {
                "all": {"future_timeframe": data_config["treatment_event_prediction_window_days"]}
            },
        }
        ...
        columns_to_keep = {
            output_naming_conventions["outcome_key"]: "max",
            # output_naming_conventions['fold_key']: 'max',
            output_naming_conventions["split_key"]: "max",
        }

        extractor_instance = DBExtractor(
            output_naming_conventions=output_naming_conventions,
            columns_to_keep=columns_to_keep,
            gt_target_keys=naming_conventions_event.keys(),
            task=task,
            gt_modes=gt_modes,
        )
        # ingest data:
        for sel in sql_select:
            ...
            extractor_instance.ingest_sql(
                db_connection_params=db_cnxn_params,
                input_naming_conventions=in_db_naming_conventions,
                event_naming_conventions=None,
                output_naming_conventions=output_naming_conventions,
                str_data=sel,
                str_external_gt=None,
                gt_target_key=None,
                same_visit_date_diff=data_config["visit_days_resolution"],
                data_source=data_config["data_source"],
            )

        # split data to train val test sets:
        splitter = SplitFilter(
            split_method="in_data",
        ...
        train_time_filt = TimeFilter(
            start_days_before=None,
            stop_days_before=time_stop,
        ...
        time_stop = data_config["days_to_inddate_tr"]
        time_start = data_config["days_to_inddate_start_tr"]
        test_time_filt = TimeFilter(
            start_days_before=time_start,
            stop_days_before=time_stop,
            date_column_name=output_naming_conventions["date_key"],
            ref_date_column_name=output_naming_conventions["index_date_key"],
        )
        ...
        extractor_instance.vectorize_inputs(
            output_naming_conventions,
            training_type,
            tr_set=train_name,
            keep_trajectories="all",
            min_visits=data_config["min_visits_per_patient"],
        )
        extractor_instance.vectorize_inputs(
            output_naming_conventions,
            training_type,
            ...
            for sel_gt_key in sql_select_gt:
                for sel_gt in sql_select_gt[sel_gt_key]:
                    extractor_instance.ingest_sql(
                        db_connection_params=db_cnxn_params,
                        input_naming_conventions=in_db_naming_conventions,
                        event_naming_conventions=naming_conventions_event[sel_gt_key],
                        output_naming_conventions=output_naming_conventions,
                        str_data=None,
                        str_external_gt=sel_gt,
                        gt_target_key=sel_gt_key,
                        data_source=data_config["data_source"],
                    )
        ...
        extractor_instance.output_to_files(
            file_config=file_config,
            tr_val_test_dirs=["train", "val", "test"],
            tr_val_tst_names=[train_name, val_name, test_name],
            format="BEHRT",
        )
