...
from ehrtransformers.model.OutcomeFuse import load_outcome_data, cleanup_vocab
...
def get_train_manager_multihead(paths, model, train_params, optim_config, head_config):
    ...
    optimizer = Bert.optimization.BertAdam(
        optimizer_grouped_parameters, lr=optim_config["lr"], warmup=optim_config["warmup_proportion"]
    )
...
def train_fuse(config_file: str):
    ...
    model_init_weights_location = model_config[
        "pretrain_model"
    ]  # None #'/data/usr/vadim/EHR/PD_ALL_SERV_270_after_ind_90_to_event_tr_90_to_event_merge_5_days/EVENT/models/PD/outcome_admdate_visit/run_102/Logs/model_dir/checkpoint_last_epoch.pth'
    ...
    load_checkpoint_mode = (
        "train"  #'infer' #'train' loads weights, epoch and learning rate. 'infer' only loads weights.
    )
    ...
    if do_train:
        logger_inst = Logger(
            file_config, model_config, data_config, global_params, argv, override_output=not global_params["debug_mode"]
        )
    else:
        logger_inst = Logger(
            file_config, model_config, data_config, global_params, argv, index=run_index, override_output=False
        )
    ...
    if True:
        ...
        head_config_inst = HeadConfig(
            head_names=model_config["heads"],
    ...
    if True:
        ...
        paths, train_params = get_paths_and_train_params_fuse(
        ...
        model = ModelMultiHead(
            conv_inputs=(
                ("data.code", 1),
                ("data.position", 1),
            ),  # (('data.input.input_0.tensor', 1),), #data.code, data.age, data.seg, data.position (defined in feature_dict)
    ...
    if do_train:
        pl_trainer, pl_module = get_train_manager_multihead(
            paths, model, train_params, optim_config, head_config=head_config_inst
        )

        if model_init_weights_location is not None:
            map_location = "cpu"
            LightningModuleDefault.load_from_checkpoint(
                model_init_weights_location,
                model_dir=paths["model_dir"],
                model=model,
    ...
    if do_infer:
        ...
        pl_module.set_predictions_keys(
            ["model.backbone_features", "data.patid", "data.vis_date"] + head_config_inst.get_add_save_outputs()
