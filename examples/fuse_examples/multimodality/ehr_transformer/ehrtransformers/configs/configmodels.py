...
class DataSettings(BaseSettings):
    days_to_inddate: Optional[
        int
    ] = 180  # days before index date when the latest visit is considered (later visits are discarded)
    days_to_inddate_start: Optional[
        int
    ] = 180  # days before index date when the first visit is considered (earlier visits are discarded)
    ...
    days_to_inddate_tr: Optional[
        int
    ] = 180  # days before index date when the latest visit is considered (later visits are discarded) in the train set (in case we want to do it differently for train and test)
    days_to_inddate_start_tr: Optional[
        int
    ] = 180  # days before index date when the earliest visit is considered (earlier visits are discarded) in the train set (in case we want to do it differently for train and test)
    ...
    use_procedures: Optional[
        bool
    ] = False  # Whether or not to use procedure codes in addition to diagnosis codes when constructing an input
    num_loader_workers: Optional[int] = 3
    visit_days_resolution: Optional[
        int
    ] = 2  # consecutive visits that are less than visit_days_resolution days apart are merged into one visit.
    ...
    data_source_str: Optional[
        str
    ] = "/data/usr/vadim/EHR/rc_for_ehr_sample.csv"  # full data source descriptor (sql code or path)
    sample_data_source_str: Optional[
        str
    ] = "/data/usr/vadim/EHR/rc_for_ehr_sample.csv"  # sample data source descriptor (sql code or path)
...
class LearningSettings(BaseSettings):
    ...
    is_data_parallel: Optional[
        bool
    ] = False  # If we use multiple GPUs, this parallellizes the application of the model across them (torch.nn.DataParallel)
    gradient_accumulation_steps: Optional[int] = 1
    save_model: Optional[bool] = True
    stop_file: Optional[str] = "stop.txt"
    optimization: Optional[OptimizationSettings] = Field(default_factory=OptimizationSettings)
...
class GlobalSettings(BaseSettings):
    ...
    debug_mode: Optional[
        bool
    ] = False  # runs in debug mode, which means that the output is directed to stdout, and not to a log file
...
class ModelSettings(BaseSettings):
    ...
    reverse_input_direction: Optional[
        bool
    ] = False  # Whether or not to reverse the input direction (i.e. first visit becomes the last visit the model sees)
    heads: Optional[list] = None
    head_weights: Optional[list] = None
    sampler_weights: Optional[list] = None
    contrastive_instances: Optional[
        int
    ] = 2  # number of patient visits used in contrastive loss (reducing distance between embeddings of the same patient)
...
class NamingConventionSettings(BaseSettings):
    ...
    adm_date_key: Optional[
        str
    ] = "ADMDATE"  # admission date - this and service date are for internal use of the db extractor only. TODO: remove them from here
    svc_date_key: Optional[
        str
    ] = "SVCDATE"  # service date - this and admission date are for internal use of the db extractor only. TODO: remove them from here
    ...
    gender_key: Optional[
        str
    ] = "SEX"  # name of a column containing gender information. TODO: separate this to input DB and internal DB versions
    ...
    split_key: Optional[
        str
    ] = "split"  # column that contains split assignment (strings 'train', 'validation' or 'test')
