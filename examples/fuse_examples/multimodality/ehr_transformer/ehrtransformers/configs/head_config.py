...
from ehrtransformers.model.OutcomeFuse import cleanup_vocab
...
from ehrtransformers.model.utils import cluster_vocab
import pandas as pd
...
def get_event_head(head_name: str, vocab_path: dict, conf, source_key: str, loss_weight: float) -> dict:
    ...
    head_dict = {
        ...
        "binarizer": mlb,
        "loss": LossDefault(
            pred="model.logits." + head_name,
            target="data." + head_name,
            callable=nn.MultiLabelSoftMarginLoss(),
            weight=loss_weight,
        ),  # Weights of the
        # different heads need not sum to 1. If they don't, it will increase
        # (or decrease) overall learning rate.
        "metrics": {
            "AUC." + head_name: FuseMetricAUC(pred_name="model.logits." + head_name, target_name="data." + head_name)
        },
        "outputs_to_save": "data." + head_name,
        "source_key": source_key,
        "vocab": event_vocab,
        "head": BertForMultiLabelPredictionHead(
            config=conf,
            head_name=head_name,
...
class HeadConfig:
    ...
    def __init__(
        ...
        if "event" in head_names:
            ...
            head_dict = get_event_head(
                head_name=head_name,
                vocab_path=get_vocab_path(
                    root_vocab_path=file_config["gt_vocab"], event_type_identifier=naming_conventions["event_key"]
                ),
                conf=conf,
                source_key="label",
        ...
        if "treatment_event" in head_names:
            ...
            head_dict = get_event_head(
                head_name=head_name,
                vocab_path=get_vocab_path(
                    root_vocab_path=file_config["gt_vocab"],
    ...
    def get_head_names(self, only_with_outputs=True):
        ...
        if only_with_outputs:
            head_names = [
                head_name for head_name in self.output_format if self.output_format[head_name]["source_key"] != None
            ]
        else:
    ...
    def get_head_inputs(self, df):
        ...
        """
        return {
            head_name: df[self.output_format[head_name]["source_key"]]
            for head_name in self.output_format
            if (
                ("source_key" in self.output_format[head_name])
                and (self.output_format[head_name]["source_key"] != None)
            )
        }
