...
class StatsAnalyzer():
    ...
    def __init__(self, input_df, naming_conventions, title=''):
        ...
        self.data_df['n_visits'] = self.data_df[self.naming['diagnosis_vec_key']].apply(lambda x : x.count(self.naming['separator_str']))

        #leave only the last visit vector:
        idx = self.data_df.groupby([self.naming['patient_id_key']])['n_visits'].transform(max) == self.data_df['n_visits']
    ...
    def get_patient_num(self, sick_healthy='all'):
        ...
        if sick_healthy == 'all':
            ...
            patient_list = set(
                self.data_df[self.data_df[self.naming['disease_key']] == self.naming['sick_val']][self.naming['patient_id_key']])
        elif sick_healthy == 'healthy':
            patient_list = set(
                self.data_df[self.data_df[self.naming['disease_key']] == self.naming['healthy_val']][self.naming['patient_id_key']])
        else:
    ...
    def print_stats(self):
        ...
        print('{:15}~{:8}~{:8}'.format(self.get_patient_num('all'),
                                self.get_patient_num('sick'),
                                self.get_patient_num('healthy')))
        ...
        for grp in ['all', 'sick', 'healthy']:
            ...
            print('{:<10}~{:10.2f}~{:10.2f}~{:10.2f}~{:10.2f}'.format(grp, mean_last_age, std_last_age, mean_first_age, std_first_age))
