...
from fuse_examples.tutorials.multimodality_image_clinical.download import (
...
def isic_2019_dataset(
    data_dir: str = "data",
    size: int = None,
    reset_cache: bool = False,
    post_cache_processing_func: Optional[Callable] = None,
):
    # data_dir = "data"
    cache_dir = "cache"
    ...
    input_processors = {
        "image": SkinInputProcessor(
            input_data=os.path.join(data_dir, "ISIC2019/ISIC_2019_Training_Input")
        ),
        ...
        "clinical": ProcessorCSV(
            csv_filename=os.path.join(
                data_dir, "ISIC2019/ISIC_2019_Training_Metadata.csv"
            ),
            sample_desc_column="image",
        ),
    }
    ...
    gt_processors = {
        "gt_global": SkinGroundTruthProcessor(
            input_data=os.path.join(
                data_dir, "ISIC2019/ISIC_2019_Training_GroundTruth.csv"
            )
        )
    }
    ...
    visualizer = VisualizerDefault(
        image_name="data.input.image",
        label_name="data.gt.gt_global.tensor",
        metadata_names=["data.input.clinical"],
        gray_scale=False,
    )

    # Create dataset
    train_dataset = DatasetDefault(
        cache_dest=cache_dir,
    ...
    validation_data_source = SkinDataSource(
        os.path.join(data_dir, "ISIC2019/ISIC_2019_Training_GroundTruth.csv"),
        size=size,
        partition_file=os.path.join(data_dir, "ISIC2019/partition.pickle"),
        train=False,
    )
