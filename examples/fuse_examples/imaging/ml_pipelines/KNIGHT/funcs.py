...
def run_train(dataset: DatasetDefault, sample_ids:Sequence, cv_index:int, test:bool=False, params:Optional[dict]=None, rep_index:int=0, rand_gen:Optional[torch.Generator]=None) -> None:
    ...
    train_dataset, validation_dataset = KNIGHT.dataset(
    ...
    losses = {
        "cls_loss": LossDefault(
            pred="model.logits.head_0", target=params["common"]["target_name"], callable=F.cross_entropy, weight=1.0
        )
    }
    ...
    train_metrics = OrderedDict(
        [
            ...
            (
                "sensitivity",
                MetricConfusion(
                    pred="results:metrics.op.cls_pred", target=params["common"]["target_name"], metrics=("sensitivity",)
                ),
            ),
        ]
    )
...
def run_infer(dataset: DatasetDefault, sample_ids:Sequence, cv_index:int, test:bool=False, params:Optional[dict]=None, rep_index:int=0, rand_gen:Optional[torch.Generator]=None) -> None:
    ...
    model = make_model(
        params["common"]["use_data"],
        params["common"]["num_classes"],
        params["train"]["imaging_dropout"],
        params["train"]["fused_dropout"],
    )
...
def run_eval(
    ...
    eval(
        target_filename=targets_filename,
        task1_prediction_filename=predictions_filename,
        task2_prediction_filename="",
