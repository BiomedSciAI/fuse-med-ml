...
def get_setting(
    ...
    TRAIN_COMMON_PARAMS[
        "data.get_selectedseries_index_func"
    ] = get_selected_series_index_radiomics

    TRAIN_COMMON_PARAMS[
        "radiomics_extractor_setting"
    ] = get_radiomics_extractor_setting2()
...
def get_X_y(a_dataset):
    ...
    for i, d in enumerate(a_dataset):
        if i == 0:
            feature_keys = sorted(
                [s for s in d.flatten() if s.startswith("data.radiomics")]
            )
            fnames = [s.replace("data.radiomics.", "") for s in feature_keys]
            is_ndarray = [
                isinstance(d[k], np.ndarray) and len(d[k].shape) > 0
        ...
        X = np.asarray(
            [
                d[k][0] if flag else float(d[k])
                for k, flag in zip(feature_keys, is_ndarray)
            ]
        )
        y_list.append(y)
        X_list.append(X)
