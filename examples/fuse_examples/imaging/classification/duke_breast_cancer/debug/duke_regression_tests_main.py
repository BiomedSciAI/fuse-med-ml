...
def main():
    ...
    sample_ids_2_test = [
        f"Breast_MRI_{i:03d}" for i in list(range(900, 901)) + [120, 596]
    ]

    cache_dir = "cache_dir_v2"
    print(
        f"============================ Comparing to {cache_dir} ========================"
    )
    test_duke(
        os.environ["DUKE_DATA_PATH"],
        cache_dir=os.path.join(duke_breast_cancer.get_duke_user_dir(), cache_dir),
        sample_ids_2_test=sample_ids_2_test,
        check_dynamic=False,
        verbose=False,
    )
...
def test_duke(root_path, cache_dir, sample_ids_2_test, check_dynamic, verbose=False):
    ...
    if check_dynamic:
        dynamic_pipeline = duke.Duke.dynamic_pipeline(
            data_dir=root_path, label_type=label_type, verbose=verbose
        )
        dict_dynamic = {}
    ...
    for s, dict_obj in to_check:
        ...
        if os.path.exists(pipeline_cache_file):
            ref_dict_obj = fuse_examples.imaging.classification.duke_breast_cancer.debug.load_object(
                pipeline_cache_file
            )
            compare_dicts(dict_obj, ref_dict_obj)
        else:
            print(
                "\tno previous results to compare to - writing current results for future tests"
            )
            fuse_examples.imaging.classification.duke_breast_cancer.debug.save_object(
                dict_obj, pipeline_cache_file
            )
