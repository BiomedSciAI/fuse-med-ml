...
def main():
    ...
    if not os.path.exists(filename):
        samples_info = get_samples_info(
            sample_ids, data_path, series_desc_2_sequence_map
        )
    ...
    annotations_df = (
        fuse_examples.imaging.classification.duke_breast_cancer.debug.get_duke_annotations_from_tal_df()
    )
...
def get_samples_info(sample_ids, data_path, series_desc_2_sequence_map):
    ...
    for sample_id in tqdm(sample_ids):
        ...
        seq_2_info_map = ops_mri.extract_seq_2_info_map(
            sample_path, series_desc_2_sequence_map
        )
