...
from fuse_examples.imaging.utils.backbone_3d_multichannel import (
...
def main():
    ...
    if False:
        ...
        selected_sample_ids = [
            f"Breast_MRI_{ii:03d}" for ii in selected_positive + selected_negative
        ]
    else:
        selected_sample_ids = None

    PATHS, TRAIN_COMMON_PARAMS, INFER_COMMON_PARAMS, EVAL_COMMON_PARAMS = get_setting(
        mode, selected_sample_ids=selected_sample_ids
    )
    print(PATHS)
    ...
    if "infer" in RUNNING_MODES:
        ...
        run_infer(
            ...
            audit_cache="train" not in RUNNING_MODES,
        )
...
def get_setting(
    ...
    if mode == "debug":
        ...
        if selected_sample_ids is None:
            selected_sample_ids = duke.get_samples_for_debug(
                data_dir=data_dir,
                n_pos=10,
                n_neg=10,
    ...
    train_folds = [
        i % n_folds for i in range(heldout_fold + 1, heldout_fold + n_folds - 1)
    ]
    ...
    INFER_COMMON_PARAMS["classification_task"] = TRAIN_COMMON_PARAMS[
        "classification_task"
    ]
...
def run_train(paths: dict, train_params: dict, reset_cache=None, audit_cache=None):
    ...
    lgr.info(
        f'validation folds={train_params["data.validation_folds"]}',
        {"color": "magenta"},
    )
    ...
    params["train"] = True
    params["cache_kwargs"] = dict(
        use_pipeline_hash=False, audit_first_sample=False, audit_rate=None
    )
    ...
    model = Fuse_model_3d_multichannel(
        ...
        backbone=ResNet(
            ...
            ch_num=train_params["backbone_model_dict"]["input_channels_num"],
        ),
        ...
        heads=[
            Head1DClassifier(
                ...
                conv_inputs=[
                    ("model.backbone_features", train_params["num_backbone_features"])
                ],
    ...
    metrics = OrderedDict(
        [
            (
                "auc",
                MetricAUCROC(
                    pred="model.output.classification", target="data.ground_truth"
                ),
            )
        ]
    )
...
######################################
def run_infer(
    paths: dict, infer_common_params: dict, audit_cache: Optional[bool] = True
):
    #### Logger
    fuse_logger_start(
    ...
    lgr.info(
        f'infer folds={infer_common_params["data.infer_folds"]}', {"color": "magenta"}
    )
    ...
    params = dict(
        label_type=infer_common_params["classification_task"],
        data_dir=paths["data_dir"],
        cache_dir=paths["cache_dir"],
        train=False,
