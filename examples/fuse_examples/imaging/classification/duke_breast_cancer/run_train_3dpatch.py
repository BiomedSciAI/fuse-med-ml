...
from fuse_examples.imaging.classification.prostate_x.backbone_3d_multichannel import (
    Fuse_model_3d_multichannel,
    ResNet,
)
from fuse_examples.imaging.classification.prostate_x.patient_data_source import (
    ProstateXDataSourcePatient,
)


from fuse_examples.imaging.classification.duke_breast_cancer.dataset import (
    duke_breast_cancer_dataset,
)
...
if TRAIN_COMMON_PARAMS["num_backbone_features_clinical"] is None:
    TRAIN_COMMON_PARAMS["num_backbone_features"] = TRAIN_COMMON_PARAMS[
        "num_backbone_features_imaging"
    ]
else:
    TRAIN_COMMON_PARAMS["num_backbone_features"] = (
        TRAIN_COMMON_PARAMS["num_backbone_features_imaging"]
        + TRAIN_COMMON_PARAMS["num_backbone_features_clinical"]
    )
...
def train_template(paths: dict, train_common_params: dict):
    ...
    fuse_logger_start(
        ...
        console_verbose_level=logging.INFO,
    ...
    # Data
    train_dataset, validation_dataset = duke_breast_cancer_dataset(
        paths, train_common_params, lgr
    )
    ...
    model = Fuse_model_3d_multichannel(
        ...
        backbone=ResNet(
            ch_num=TRAIN_COMMON_PARAMS["backbone_model_dict"]["input_channels_num"]
        ),
    ...
    losses = {
        "cls_loss": LossDefault(
            pred="model.logits.isLargeTumorSize",
    ...
    metrics = {
        "auc": MetricAUCROC(
            pred="model.output.isLargeTumorSize",
            target="data.ground_truth",
...
INFER_COMMON_PARAMS["infer_filename"] = os.path.join(
    PATHS["inference_dir"], "validation_set_infer.pickle.gz"
)
INFER_COMMON_PARAMS[
    "checkpoint"
...
def infer_template(paths: dict, infer_common_params: dict):
    ...
    ## Create data source:
    infer_data_source = ProstateXDataSourcePatient(
        paths["data_dir"],
        "validation",
    ...
    dataset = DatasetBase.load(
        filename=data_set_filename,
        override_datasource=infer_data_source,
        override_cache_dest=paths["cache_dir"],
        num_workers=0,
    )
...
def eval_template(paths: dict, eval_common_params: dict):
    ...
    metrics = {
        ...
        "auc": MetricAUCROC(
            pred="model.output.isLargeTumorSize", target="data.ground_truth"
        ),
    }
