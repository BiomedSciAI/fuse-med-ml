...
from fuse_examples.imaging.classification.duke_breast_cancer.post_processor import (
    post_processing,
)
from fuse_examples.imaging.classification.duke_breast_cancer.processor import (
    PatchProcessor,
)
...
def process_mri_series(metadata_path: str):
    ...
    for opt_seq_tmp, my_key in zip(opt_seq, my_keys):
        tt = [s for s in seq_to_use_full if opt_seq_tmp in s] + [
            s for s in seq_to_use_full_slash if opt_seq_tmp in s
        ]
...
def duke_breast_cancer_dataset(paths, train_common_params, lgr):
    ...
    mri_vol_processor = DicomMRIProcessor(
        seq_dict=seq_dict,
    ...
    generate_processor = PatchProcessor(
        ...
        fold_no=train_common_params["fold_no"],
        lsn_shape=(
            image_processing_args["patch_z"],
            image_processing_args["patch_xy"],
            image_processing_args["patch_xy"],
        ),
    )

    train_post_processor = partial(
        post_processing, label=train_common_params["classification_task"]
    )
    ...
    aug_pipeline = [
        ...
        [("data.input",), squeeze_3d_to_2d, {"axis_squeeze": "z"}, {}],
        [
            ("data.input",),
            aug_op_affine,
            {
    ...
    visualizer = Fuse3DVisualizerDefault(
        image_name="data.input", label_name="data.isLargeTumorSize"
    )
    # Create dataset
    ...
    ## Create data source
    validation_data_source = ProstateXDataSourcePatient(
        paths["data_dir"],
        "validation",
    ...
    # post processor
    validation_post_processor = partial(
        post_processing, label=train_common_params["classification_task"]
    )
