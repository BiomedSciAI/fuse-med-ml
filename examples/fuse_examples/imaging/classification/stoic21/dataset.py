...
def create_dataset(train_val_sample_ids: Union[Sequence, None]=None, paths: Optional[dict]=None, params: Optional[dict]=None) -> Sequence[DatasetDefault]:
    if train_val_sample_ids is None:
        ...
        train_val_dataset = STOIC21.dataset(paths["data_dir"], paths["cache_dir"], sample_ids=train_val_sample_ids, train=True) # note all of this dataset will get a "training" pipeline (with augmentations etc')
        test_dataset = STOIC21.dataset(
            paths["data_dir"], paths["cache_dir"], sample_ids=test_sample_ids, train=False
        )
        return train_val_dataset, test_dataset
    else:
        train_dataset = STOIC21.dataset(paths["data_dir"], paths["cache_dir"], sample_ids=train_val_sample_ids[0], train=True) 
        validation_dataset = STOIC21.dataset(paths["data_dir"], paths["cache_dir"], sample_ids=train_val_sample_ids[1], train=False) 
