...
def run_train(paths: NDict, train: NDict):
    ...
    callbacks = [
        ...
        TimeStatisticsCallback(
            num_epochs=train["manager_train_params"]["num_epochs"],
    ...
    manager = ManagerDefault(
        output_model_dir=paths["model_dir"], force_reset=train["force_reset_model_dir"]
    )
    ...
    if train["resume_checkpoint_filename"] is not None:
        ...
        manager.load_checkpoint(
            checkpoint=train["resume_checkpoint_filename"],
            mode="train",
...
def run_infer(paths: NDict, infer: NDict):
    ...
    lgr.info(
        f'infer_filename={os.path.join(paths["inference_dir"], infer["infer_filename"])}',
        {"color": "magenta"},
    )
...
def main(cfg: DictConfig) -> None:
    ...
    force_gpus = None  # [0]
    choose_and_enable_multiple_gpus(
        cfg["train.manager_train_params.num_gpus"], force_gpus=force_gpus
    )
