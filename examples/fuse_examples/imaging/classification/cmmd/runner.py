...
def run_train(paths: dict, train_common_params: dict, reset_cache: bool):
    ...
    train_dataset, validation_dataset, _ = CMMD_2021_dataset(
        paths["data_dir"], paths["data_misc_dir"], reset_cache=reset_cache
    )
...
def run_infer(paths: dict, infer_common_params: dict):
    ...
    # Create data source:
    _, _, test_dataset = CMMD_2021_dataset(
        paths["data_dir"], paths["data_misc_dir"], paths["cache_dir"]
    )
...
if __name__ == "__main__":
    ...
    ), "Error: please set root_data, the path to the stored CMMD dataset location"
    ...
    if "train" in RUNNING_MODES:
        run_train(
            paths=paths, train_common_params=TRAIN_COMMON_PARAMS, reset_cache=False
        )
