...
class MGInputProcessor(ProcessorBase):
    ...
    def __call__(self, inner_image_desc, *args, **kwargs):
        ...
        inner_image = standardize_breast_image(
            inner_image, self.normalized_target_range
        )

        # resize
        if self.resize_to is not None:
            inner_image = ProcessorsImageToolBox.resize_image(
                inner_image, self.resize_to
            )

        # padding
        if self.padding is not None:
            image = ProcessorsImageToolBox.pad_image(
                inner_image,
                self.padding,
                self.resize_to,
...
def find_breast_aabb(
    img: np.ndarray, dark_area_threshold: int = 10, blocks_num: int = 30
    ...
    cells_binary[
        cells > dark_area_threshold
    ] = True  # TODO: this is a hardcoded threshold, see if there's a better alternative
...
def check_breast_side_is_left_MG(
    image: np.ndarray, max_pixel_value=255.0, dark_region_ratio=15.0
) -> bool:
    """
...
def standardize_breast_image(
    inner_image: np.ndarray, normalized_target_range: Tuple[float, float]
) -> np.ndarray:
    """
    ...
    # normalize
    inner_image = ProcessorsImageToolBox.normalize_to_range(
        inner_image, range=normalized_target_range
    )
