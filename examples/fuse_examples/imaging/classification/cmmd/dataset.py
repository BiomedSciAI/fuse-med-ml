...
from fuse_examples.imaging.classification.cmmd.ground_truth_processor import (
    MGGroundTruthProcessor,
)
...
def CMMD_2021_dataset(
    data_dir: str,
    data_misc_dir: str,
    cache_dir: str = "cache",
    reset_cache: bool = False,
    post_cache_processing_func: Optional[Callable] = None,
) -> Tuple[DatasetDefault, DatasetDefault]:
    """
    ...
    input_source_gt = merge_clinical_data_with_dicom_tags(
        data_dir, data_misc_dir, target
    )

    partition_file_path = os.path.join(data_misc_dir, "data_fold_new.csv")
    train_data_source = DataSourceFolds(
        input_source=input_source_gt,
        input_df=None,
        phase="train",
        no_mixture_id="ID1",
        balance_keys=[target],
    ...
    input_processors = {"image": MGInputProcessor(input_data=data_dir)}
    gt_processors = {
    ...
    visualiser = VisualizerDefault(
        image_name="data.input.image", label_name="data.gt.classification"
    )
    ...
    validation_data_source = DataSourceFolds(
        input_source=input_source_gt,
        input_df=None,
        phase="validation",
        no_mixture_id="ID1",
    ...
    test_data_source = DataSourceFolds(
        input_source=input_source_gt,
        input_df=None,
        phase="test",
        no_mixture_id="ID1",
