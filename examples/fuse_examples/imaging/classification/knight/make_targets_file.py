...
def make_targets_file(
    ...
    if is_validation_set:
        ...
        labels.insert(
            1, "Task1-target", labels["Task2-target"].apply(lambda x: int(x > 3))
        )
    ...
    if is_validation_set:
        targets_df = ExportDataset.export_to_dataframe(
            ...
            [
                "data.descriptor",
                "data.gt.gt_global.task_1_label",
                "data.gt.gt_global.task_2_label",
            ],
        )
    else:  # test set
        targets_df = ExportDataset.export_to_dataframe(
    ...
    targets_df["data.gt.gt_global.task_1_label"] = targets_df[
        "data.gt.gt_global.task_1_label"
    ].transform(int)
    targets_df["data.gt.gt_global.task_2_label"] = targets_df[
        "data.gt.gt_global.task_2_label"
    ].transform(int)
