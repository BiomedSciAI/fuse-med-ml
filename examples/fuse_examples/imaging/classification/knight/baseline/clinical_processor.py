...
class KiCClinicalProcessor(ProcessorBase):
    ...
    def __call__(self, sample_desc: Hashable):
        ...
        if sample_data["last_preop_egfr"] is None:
            sample_data[
                "last_preop_egfr"
            ] = 77  # this is the median value on the training set
        else:
    ...
    def convert_to_tensor(
        sample: dict, key: str, tensor_dtype: Optional[str] = None
    ) -> None:
        """
        ...
        if key not in sample:
            ...
            lgr.error(
                f"Column {key} does not exit in dataframe, it is ignored and not converted to {tensor_dtype}"
            )
        else:
...
class KiCGTProcessor(ProcessorBase):
    ...
    def __call__(self, sample_desc: Hashable):
        ...
        # process fields
        # Task 1 labels:
        if sample_data["aua_risk_group"] in [
            "high_risk",
            "very_high_risk",
