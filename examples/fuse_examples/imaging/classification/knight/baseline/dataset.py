...
def knight_dataset(
    ...
    if "train" in split:
        ...
        gt_processors = {
            "gt_global": KiCGTProcessor(
                json_filename=json_filepath,
                columns_to_tensor={
                    "task_1_label": torch.long,
                    "task_2_label": torch.long,
                },
            )
        }
        ...
        if only_labels:
            ...
            gt_processors = {
                "gt_global": KiCGTProcessor(
                    json_filename=json_labels_filepath,
                    columns_to_tensor={
                        "task_1_label": torch.long,
                        "task_2_label": torch.long,
                    },
    ...
    visualizer = Fuse3DVisualizerDefault(
        image_name="data.input.image", label_name=target_name
    )
    # Create dataset
    if "train" in split:
        ...
        sampler = SamplerBalancedBatch(
            ...
            balanced_class_probs=[1.0 / num_classes] * num_classes
            if task_num == 2
            else None,
            use_dataset_cache=False,
        )  # we don't want to use_dataset_cache here since it's more
        ...
        train_dataloader = (
            train_dataset
        ) = validation_dataloader = validation_dataset = None
    return (
        train_dataloader,
