...
from fuse_examples.imaging.classification.skin_lesion.input_processor import (
    SkinInputProcessor,
)
from fuse_examples.imaging.classification.skin_lesion.ground_truth_processor import (
    SkinGroundTruthProcessor,
)
from fuse_examples.imaging.classification.skin_lesion.download import (
...
TRAIN_COMMON_PARAMS["data.augmentation_pipeline"] = [
    ...
    [("data.input.input_0",), aug_op_gaussian, {"std": 0.03}, {"apply": RandBool(0.7)}],
]

assert (
    TRAIN_COMMON_PARAMS["data.year"] == "2016"
    or TRAIN_COMMON_PARAMS["data.year"] == "2017"
)
...
def run_train(paths: dict, train_common_params: dict):
    ...
    input_source_gt = {
        "2016": os.path.join(
            paths["data_dir"], "data/ISIC2016_Training_GroundTruth.csv"
        ),
        "2017": os.path.join(
            paths["data_dir"], "data/ISIC2017_Training_GroundTruth.csv"
        ),
    }
    training_data = {
        "2016": os.path.join(paths["data_dir"], "data/ISIC2016_Training_Data/"),
        "2017": os.path.join(paths["data_dir"], "data/ISIC2017_Training_Data/"),
    }
    input_source_gt_val = {
        "2016": input_source_gt["2016"],
        "2017": os.path.join(
            paths["data_dir"], "data/ISIC2017_Validation_GroundTruth.csv"
        ),
    }
    validation_data = {
        "2016": training_data["2016"],
        "2017": os.path.join(paths["data_dir"], "data/ISIC2017_Validation_Data/"),
    }

    partition_file = {
        "2016": "train_val_split.pickle",
        "2017": None,
    }  # as no validation set is available in 2016 dataset, it needs to be created

    train_data_source = SkinDataSource(
        input_source_gt[train_common_params["data.year"]],
    ...
    input_processors = {
        "input_0": SkinInputProcessor(
            input_data=training_data[train_common_params["data.year"]]
        )
    }
    gt_processors = {
        "gt_global": SkinGroundTruthProcessor(
            input_data=input_source_gt[train_common_params["data.year"]],
    ...
    visualizer = VisualizerDefault(
        image_name="data.input.input_0", label_name="data.gt.gt_global.tensor"
    )

    # Create dataset
    train_dataset = DatasetDefault(
        cache_dest=paths["cache_dir"],
    ...
    ## Create data source
    validation_data_source = SkinDataSource(
        input_source_gt_val[train_common_params["data.year"]],
        partition_file=partition_file[train_common_params["data.year"]],
        train=False,
    )
    ...
    input_processors = {
        "input_0": SkinInputProcessor(
            input_data=validation_data[train_common_params["data.year"]]
        )
    }
    gt_processors = {
        "gt_global": SkinGroundTruthProcessor(
            input_data=input_source_gt_val[train_common_params["data.year"]],
            year=train_common_params["data.year"],
        )
    }
    ...
    validation_dataset.create(
        pool_type="thread"
    )  # use ThreadPool to create this dataset, to avoid cv2 problems in multithreading
    lgr.info(f"- Load and cache data: Done")
    ...
    model = ModelDefault(
        ...
        backbone={
            ...
            "InceptionResnetV2": BackboneInceptionResnetV2(
                input_channels_num=3, logical_units_num=43
            ),
        }["InceptionResnetV2"],
        heads=[
            HeadGlobalPoolingClassifier(
    ...
    metrics = OrderedDict(
        [
            ...
            (
                "auc",
                MetricAUCROC(
                    pred="model.output.head_0", target="data.gt.gt_global.tensor"
                ),
            ),
            (
                "accuracy",
...
INFER_COMMON_PARAMS["data.train_num_workers"] = TRAIN_COMMON_PARAMS[
...
def run_infer(paths: dict, infer_common_params: dict):
    ...
    # Create data source:
    input_source = {
        "2016": os.path.join(paths["data_dir"], "data/ISIC2016_Test_GroundTruth.csv"),
        "2017": os.path.join(paths["data_dir"], "data/ISIC2017_Test_GroundTruth.csv"),
    }
    infer_data = {
        "2016": os.path.join(paths["data_dir"], "data/ISIC2016_Test_Data/"),
    ...
    input_processors_infer = {
        "input_0": SkinInputProcessor(
            input_data=infer_data[infer_common_params["data.year"]]
        )
    }
    gt_processors_infer = {
        "gt_global": SkinGroundTruthProcessor(
            input_data=input_source[infer_common_params["data.year"]],
            train=False,
            year=infer_common_params["data.year"],
        )
    }
