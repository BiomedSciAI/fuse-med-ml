...
def main():
    ...
    dataset = prostate_x.ProstateX.dataset(
        label_type=label_type, train=False, cache_dir=cache_dir, data_dir=data_dir
    )
...
def test_tals_data(df):
    ...
    dd = df.groupby(["Patient ID"])[["fid", "is_ClinSig"]].agg(
        {"fid": "count", "is_ClinSig": "mean"}
    )
...
def read_and_test_tal_folds():
    ...
    n_patient_fold_pairs = (
        df_all["Patient ID"] + "_" + df_all["fold"].astype(str)
    ).nunique()
