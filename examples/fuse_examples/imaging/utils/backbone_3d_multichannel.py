...
class ResNet(torch.nn.Module):
    ...
    def forward(self, batch_dict: NDict):
        if len(self.conv_inputs) > 1:
            tensors_list = [
                batch_dict[conv_input[0]].float() for conv_input in self.conv_inputs
            ]
            max_tensor_dim = max([len(tmp_tensor.shape) for tmp_tensor in tensors_list])
            conv_input = torch.cat(
                [
                    tmp_tensor.unsqueeze_(0)
                    if len(tmp_tensor.shape) < max_tensor_dim
                    else tmp_tensor
...
if __name__ == "__main__":
    ...
    with gzip.open(real_data_example, "rb") as pickle_file:
        ...
        dummy_data["data.input"] = torch.cat(
            (input.unsqueeze(0), input.unsqueeze(0)), dim=0
        )
        dummy_data["data.tensor_clinical"] = torch.cat(
            (zone2feature[zone], zone2feature[zone]), dim=0
        )
    ...
    if not DATAPARALLEL:
        backbone_params = sum(
            p.numel() for p in model._modules["backbone_clinical"].parameters()
        )
