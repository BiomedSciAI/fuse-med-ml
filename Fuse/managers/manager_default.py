...
class FuseManagerDefault:
    ...
    def load_objects(self, input_model_dir: Union[str, Sequence[str]], list_of_object_names: List[str] = None, mode: str = 'infer') -> Dict[str, Any]:
        ...
        if mode == 'infer':
            inference_dataset_filename = os.path.join(input_model_dir, "inference_dataset.pth")
            import pickle
            with open(inference_dataset_filename, 'rb') as pickle_file:
                inference_dataset = pickle.load(pickle_file)
            loaded_objects['inference_dataset'] = inference_dataset
    ...
    def infer(self, input_model_dir: Optional[Union[str, Sequence]] = None,
        ...
        if input_model_dir is not None:
            ...
            if not hasattr(self.state, 'net'):
                self.load_objects(input_model_dir, mode='infer')  # this method can load either a single model or an ensemble
