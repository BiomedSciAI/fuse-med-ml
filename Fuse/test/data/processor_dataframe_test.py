...
class FuseProcessorDataFrameTestCase(unittest.TestCase):
    ...
    def test_all_columns(self):
        ...
        self.assertDictEqual(proc('one'), {'desc':'one', 'int_val': 4, 'string_val':'val1'})
        self.assertDictEqual(proc('two'), {'desc':'two', 'int_val': 5, 'string_val':'val2'})
        self.assertDictEqual(proc('three'), {'desc':'three', 'int_val': 6, 'string_val':'val3'})

    def test_rename(self):
        df = pd.DataFrame({"desc": ['one', 'two', 'three'], "int_val": [4, 5, 6], "string_val": ['val1', 'val2', 'val3']})
        proc = FuseProcessorDataFrame(data=df, sample_desc_column='desc', rename_columns={'int_val': 'new_name'})

        self.assertDictEqual(proc('one'), {'desc':'one', 'new_name': 4, 'string_val':'val1'})
        self.assertDictEqual(proc('two'), {'desc':'two', 'new_name': 5, 'string_val':'val2'})
        self.assertDictEqual(proc('three'), {'desc':'three', 'new_name': 6, 'string_val':'val3'})

    def test_specific_columns(self):
        df = pd.DataFrame({"desc": ['one', 'two', 'three'], "int_val": [4, 5, 6], "string_val": ['val1', 'val2', 'val3']})
        proc = FuseProcessorDataFrame(data=df, sample_desc_column='desc', rename_columns={'int_val': 'new_name'},
                                      columns_to_extract=['string_val'])
    ...
    def test_tensors(self):
        ...
        self.assertDictEqual(proc('one'), {'desc':'one', 'int_val': torch.tensor(4, dtype=torch.int64), 'string_val':'val1'})
        self.assertDictEqual(proc('two'), {'desc':'two', 'int_val': torch.tensor(5, dtype=torch.int64), 'string_val':'val2'})
        self.assertDictEqual(proc('three'), {'desc':'three', 'int_val': torch.tensor(6, dtype=torch.int64), 'string_val':'val3'})
    ...
    def test_tensors_with_types(self):
        ...
        self.assertDictEqual(proc('one'), {'desc':'one', 'int_val': torch.tensor(4, dtype=torch.int8),
                                           'float_val': torch.tensor(4.1, dtype=torch.float64), 'string_val':'val1'})
        self.assertDictEqual(proc('two'), {'desc':'two', 'int_val': torch.tensor(5, dtype=torch.int8),
                                           'float_val': torch.tensor(5.3, dtype=torch.float64), 'string_val':'val2'})
        self.assertDictEqual(proc('three'), {'desc':'three', 'int_val': torch.tensor(6, dtype=torch.int8),
