...
class Activation(nn.Module):
    def __init__(self, name: str, **params: Any):
        ...
        if name is None or name == "identity":
            ...
            raise ValueError(
                f"Activation should be callable/sigmoid/softmax/logsoftmax/None; got {name}"
            )
...
class EfficientNetEncoder(EfficientNet, EncoderMixin):
    def __init__(
        self, stage_idxs: tuple, out_channels: tuple, model_name: str, depth: int = 5
    ):
...
def get_encoder(
    name: str, in_channels: int = 3, depth: int = 5, weights: str | None = None
) -> nn.Module:
