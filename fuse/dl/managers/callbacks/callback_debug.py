...
class CallbackDebug(Callback):
    ...
    def on_step_end(
        ...
        if self.first_step:
            ...
            lgr.info(
                f"Step {step} - {current_time} (total time {total_time}) - END",
                {"color": "green", "attrs": ["bold", "underline"]},
            )

            lgr.info("Step Results:", {"color": "green", "attrs": "bold"})
            lgr.info(f"Learning Rate: {learning_rate}", {"color": "green"})
            lgr.info("Train Results:", {"color": "green", "attrs": "bold"})
            lgr.info(f"{train_results}", {"color": "green"})
            lgr.info("Validation Results:", {"color": "green", "attrs": "bold"})
    ...
    def on_epoch_begin(self, mode: str, epoch: int) -> None:
        ...
        if self.first_epoch.get(mode, True):
            ...
            logging.getLogger("Fuse").info(
                f"\n{mode.capitalize()} epoch {epoch} - {current_time} - BEGIN",
                {"color": "green", "attrs": ["bold", "underline"]},
            )
    ...
    def on_epoch_end(self, mode: str, epoch: int, epoch_results: Dict = None) -> None:
        ...
        if self.first_epoch.get(mode, True):
            ...
            lgr.info(
                f"{mode.capitalize()}  epoch {epoch} - {current_time} (total time {total_time}) - END",
                {"color": "green", "attrs": ["bold", "underline"]},
            )

            lgr.info("Epoch Results:", {"color": "green", "attrs": "bold"})
    ...
    def on_virtual_batch_begin(self, mode: str, virtual_batch: int) -> None:
        ...
        if self.first_virtual_batch.get(mode, True):
            ...
            logging.getLogger("Fuse").info(
                f"\n{mode.capitalize()} virtual batch {virtual_batch} - {current_time} - BEGIN",
                {"color": "green", "attrs": ["bold", "underline"]},
            )
    ...
    def on_virtual_batch_end(self, mode: str, virtual_batch: int, virtual_batch_results: Dict = None) -> None:
        ...
        if self.first_virtual_batch.get(mode, True):
            ...
            lgr.info(
                f"{mode.capitalize()} virtual batch {virtual_batch} - {current_time} (total time {total_time}) - END",
                {"color": "green", "attrs": ["bold", "underline"]},
            )

            lgr.info("Virtual Batch Results:", {"color": "green", "attrs": "bold"})
    ...
    def on_batch_begin(self, mode: str, batch: int) -> None:
        ...
        if self.first_batch.get(mode, True):
            ...
            logging.getLogger("Fuse").info(
                f"\n{mode.capitalize()} batch {batch} - {current_time} - BEGIN",
                {"color": "green", "attrs": ["bold", "underline"]},
            )
    ...
    def on_data_fetch_end(self, mode: str, batch: int, batch_dict: Dict = None) -> None:
        ...
        if self.first_batch.get(mode, True):
            ...
            lgr.info(
                f"{mode.capitalize()} data fetch {batch} - {current_time} (total time {total_time}) - END",
                {"color": "green", "attrs": ["bold", "underline"]},
            )

            lgr.info("Batch Dict:", {"color": "green", "attrs": "bold"})
    ...
    def on_batch_end(self, mode: str, batch: int, batch_dict: Dict = None) -> None:
        ...
        if self.first_batch.get(mode, True):
            ...
            total_time = time_batch_end - self.time_batch_begin
            lgr.info(
                f"{mode.capitalize()} batch {batch} - {current_time} (total time {total_time}) - END",
                {"color": "green", "attrs": ["bold", "underline"]},
            )
