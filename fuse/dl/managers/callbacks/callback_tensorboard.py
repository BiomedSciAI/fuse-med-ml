...
class TensorboardCallback(Callback):
    ...
    def on_step_end(
        self,
        step: int,
        train_results: NDict = None,
        validation_results: NDict = None,
        ...
        for evaluator_name in train_results.flatten().keys():
            ...
            if evaluator_value is not None and isinstance(
                evaluator_value, (int, float, np.ndarray, torch.Tensor)
            ):
                self.add_scalar(
                    self.tensorboard_logger_train,
                    tag=evaluator_name,
                    scalar_value=evaluator_value,
                    global_step=step,
                )
        self.add_scalar(
            self.tensorboard_logger_train, "learning_rate", learning_rate, step
        )
        ...
        if validation_results is not None:
            ...
            self.add_scalar(
                self.tensorboard_logger_validation, "learning_rate", learning_rate, step
            )
    ...
    def on_train_begin(self, state: ManagerState) -> None:
        ...
        self.tensorboard_logger_validation = self.writer_class(
            tensorboard_validation_dir
        )
        pass
    ...
    def add_scalar(self, writer, tag, scalar_value, global_step):
        ...
        if scalar_value is not None and not isinstance(scalar_value, str):
            if self.use_summary_tf:  # FileWrite
                summary = self.tf_summary(
                    value=[self.tf_summary.Value(tag=tag, simple_value=scalar_value)]
                )
