...
class ManagerDefault:
    ...
    def __init__(self, output_model_dir: str = None, force_reset: bool = False):
        ...
        if output_model_dir is not None:
            ...
            create_or_reset_dir(
                output_model_dir,
                ignore_files=["logs", "source_files"],
    ...
    def set_objects(
        ...
        if FuseDebug().get_setting("manager_stages") != "default":
            ...
            self.logger.info(
                f"Manager - debug mode - append debug callback", {"color": "red"}
            )
        pass
    ...
    def _save_objects(self) -> None:
        ...
        def _torch_save(parameter_to_save: Any, parameter_name: str) -> None:
            file_path = os.path.join(
                self.state.output_model_dir, f"{parameter_name}.pth"
            )
    ...
    def load_objects(
        self,
        input_model_dir: Union[str, Sequence[str]],
        list_of_object_names: List[str] = None,
        mode: str = "infer",
    ) -> Dict[str, Any]:
        """
        ...
        if isinstance(
            input_model_dir, (tuple, list)
        ):  # if multiple model dirs, load ensemble modules
            if mode != "infer":
            ...
            for model_idx, model_dir in enumerate(input_model_dir):
                self.logger.info(
                    "Loading ensemble model %d from: %s" % (model_idx, model_dir)
                )
        ...
        if mode == "infer":
            load_if_exists("inference_dataset") if should_load(
                "inference_dataset"
            ) else self.state.metrics
            return loaded_objects

        self.state.metrics = (
            load_if_exists("metrics") if should_load("metrics") else self.state.metrics
        )
        self.state.losses = (
            load_if_exists("losses") if should_load("losses") else self.state.losses
        )
        self.callbacks = (
            load_if_exists("callbacks") if should_load("callbacks") else self.callbacks
        )
        self.state.optimizer = (
            load_if_exists("optimizer")
            if should_load("optimizer")
            else self.state.optimizer
        )
        self.state.lr_scheduler = (
            load_if_exists("lr_scheduler")
            if should_load("lr_scheduler")
            else self.state.lr_scheduler
        )
        self.state.best_epoch_source = (
            load_if_exists("best_epoch_source")
            if should_load("best_epoch_source")
            else self.state.best_epoch_source
        )
        self.state.train_params = (
            load_if_exists("train_params")
            if should_load("train_params")
            else self.state.train_params
        )
    ...
    def load_checkpoint(
        ...
        checkpoints = (
            [checkpoint] if not isinstance(checkpoint, (list, tuple)) else checkpoint
        )

        if model_dir is not None:
            model_dirs = (
                [model_dir] if not isinstance(model_dir, (list, tuple)) else model_dir
            )
        ...
        for idx in range(len(checkpoints)):
            ...
            if isinstance(checkpoint_desc, int) or (
                isinstance(checkpoint_desc, str)
                and (checkpoint_desc in ["best", "last"])
            ):
                if isinstance(checkpoint_desc, str) and (checkpoint_desc == "best"):
                    checkpoint_desc = checkpoint_desc + "_" + str(index)
                checkpoint_file = os.path.join(
                    model_dirs[idx], f"checkpoint_{checkpoint_desc}_epoch.pth"
                )
            ...
            self.logger.info(
                f"Loading checkpoint file: {checkpoint_file}. values_to_resume {str_vals}",
                {"color": "yellow"},
            )
        ...
        if mode == "train":
            ...
            if should_load("start_epoch"):
                ...
                self.logger.info(
                    f"Loaded start epoch: {self.state.current_epoch}",
                    {"color": "yellow"},
                )
            ...
            if should_load("learning_rate"):
                ...
                self.logger.info(
                    f"Loaded learning rate: {self.state.learning_rate}",
                    {"color": "yellow"},
                )
    ...
    def train(
        self, train_dataloader: DataLoader, validation_dataloader: DataLoader = None
    ) -> None:
        """
        ...
        # debug - num workers
        override_num_workers = FuseDebug().get_setting(
            "manager_override_num_dataloader_workers"
        )
        if override_num_workers != "default":
            ...
            self.logger.info(
                f"Manager - debug mode - override dataloader num_workers to {override_num_workers}",
                {"color": "red"},
            )
        ...
        trainable_param = sum(
            p.numel() for p in self.state.net.parameters() if p.requires_grad
        )
        self.logger.info(
        ...
        self.state.best_epoch_values = [
            initial_results for i in range(self.state.num_models_to_save)
        ]
        ...
        while self.state.current_epoch < self.state.end_epoch:
            ...
            train_results = self.handle_epoch(
                "train", self.state.current_epoch, train_dataloader
            )

            # validation epoch - only if validation is needed
            if validation_dataloader is not None:
                self.logger.info(
                    f"Start validation on epoch {self.state.current_epoch}"
                )
                validation_results = self.handle_epoch(
                    "validation", self.state.current_epoch, validation_dataloader
                )
            else:
            ...
            epoch_checkpoint = Checkpoint(
                state_dict, self.state.current_epoch, self.get_current_learning_rate()
            )

            # if this is the best epoch yet
            for i in range(self.state.num_models_to_save):
                if (
                    self._is_best_epoch_so_far(train_results, validation_results, i)
                    or self.state.current_epoch == 1
                ):  # consider the first epoch as the best one
                    best_val = self.state.best_epoch_values[i][
                        self.state.best_epoch_function[i]
                    ]
                    self.logger.info(
                    ...
                    best_epoch_checkpoint_filename = os.path.join(
                        self.state.output_model_dir,
                        "checkpoint_best_" + str(i) + "_epoch.pth",
                    )
            ...
            last_epoch_checkpoint_filename = os.path.join(
                self.state.output_model_dir, "checkpoint_last_epoch.pth"
            )
            ...
            if self.is_epoch_for_save(self.state.current_epoch):
                this_epoch_checkpoint_filename = os.path.join(
                    self.state.output_model_dir,
                    f"checkpoint_{self.state.current_epoch}_epoch.pth",
                )
    ...
    def infer(
        ...
        if append_default_inference_callback:
            self.callbacks.append(
                InferResultsCallback(
                    output_file=output_file_name, output_columns=output_columns
                )
            )
    ...
    def do_handle_epoch(self, mode: str, epoch: int, data_loader: DataLoader) -> NDict:
        ...
        for virtual_batch in trange(num_batches):
            ...
            virtual_batch_dict = self.handle_virtual_batch(
                mode, virtual_batch, data_iter
            )
            epoch_results = _extend_results_dict(
                mode, virtual_batch_dict, epoch_results
            )
    ...
    def handle_virtual_batch(
        self, mode: str, virtual_batch: int, data_iter: Iterator
    ) -> NDict:
        """
        ...
        for mini_batch in range(self.state.virtual_batch_size):
            ...
            virtual_batch_results = _extend_results_dict(
                mode, mini_batch_result_dict, virtual_batch_results
            )
    ...
    def _verify_all_objects_initialized(self, mode: str):
        ...
        if mode == "train":
            ...
            if override_num_epochs != "default":
                ...
                self.logger.info(
                    f"Manager - debug mode - override num_epochs to {self.state.num_epochs}",
                    {"color": "red"},
                )

            self.state.start_saving_epochs: int = full_config["start_saving_epochs"]
            self.state.gap_between_saving_epochs: int = full_config[
                "gap_between_saving_epochs"
            ]
            ...
            self.state.num_models_to_save = (
                1
                if isinstance(self.state.best_epoch_source, dict)
                else len(self.state.best_epoch_source)
            )
            ...
            for i in range(self.state.num_models_to_save):
                ...
                if self.state.metrics:
                    self.state.best_epoch_function.append(
                        self.state.best_epoch_source[i].get(
                            "source", f"metrics.{list(self.state.metrics.keys())[0]}"
                        )
                    )
                else:
                    self.state.best_epoch_function.append(
                        self.state.best_epoch_source[i].get(
                            "source", f"losses.{list(self.state.losses.keys())[0]}"
                        )
                    )

                self.state.optimization_function.append(
                    self.state.best_epoch_source[i].get("optimization", "max")
                )
                assert self.state.optimization_function[i] in ["max", "min"]
                initial_value = (
                    -float("inf")
                    if self.state.optimization_function[i] == "max"
                    else float("inf")
                )
                self.state.best_epoch_values.append(
                    {self.state.best_epoch_function[i]: initial_value}
                )

                on_equal_values = self.state.best_epoch_source[i].get(
    ...
    def is_epoch_for_save(self, epoch: int) -> bool:
        ...
        return (
            ...
            and (epoch - self.state.start_saving_epochs)
            % self.state.gap_between_saving_epochs
            == 0
        )

    def _is_best_epoch_so_far(
        self, train_results: NDict, validation_results: NDict, epoch_source_index: int
    ) -> bool:
        """
        ...
        def is_better_epoch_value(current_value: float) -> bool:
            try:
                ...
                if current_value is None:
                    lgr = logging.getLogger("Fuse")
                    lgr.error(
                        f"Comparing epochs failed since value is None, assuming it is not the best epoch"
                    )
    ...
    def _write_epoch_summary_table(
        self, train_dict: NDict, validation_dict: dict, epoch_source_index: int
    ) -> None:
        ...
        for evaluator_name in eval_keys:
            ...
            best_so_far_str = get_value_as_float_str(
                self.state.best_epoch_values[epoch_source_index], evaluator_name
            )
        ...
        try:
            op = (
                "w" if epoch_source_index == 0 else "a"
            )  # we want to have all stats in one file per epoch
            with open(
                os.path.join(self.state.output_model_dir, "last_epoch_summary.txt"), op
            ) as sfile:
    ...
    def set_config_defaults(self, config: dict, mode: str) -> Dict:
        ...
        def set_default(key, value):
            if key not in full_config:
                ...
                self.logger.info(
                    f"Key {key} not found in config parameter, setting value to default ({value})",
                    {"color": "yellow"},
                )
    ...
    def _handle_dataset_summaries(
        ...
        train_dataset_summary_file = os.path.join(
            self.state.output_model_dir, "train_dataset_summary.txt"
        )
...
def _extend_results_dict(
    mode: str, current_dict: NDict, aggregated_dict: NDict
) -> NDict:
    """
