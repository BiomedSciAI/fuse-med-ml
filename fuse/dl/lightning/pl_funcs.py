...
def model_checkpoint_callbacks(
    model_dir: str, best_epoch_source: Union[Dict, List[Dict]]
) -> List[pl.Callback]:
    """
    ...
    callbacks.append(
        ModelCheckpoint(dirpath=model_dir, filename="last_epoch", save_last=True)
    )
    return callbacks
...
def epoch_end_compute_and_log_losses(
    pl: pl.LightningModule, mode: str, batch_losses: Sequence[Dict]
) -> None:
    """
...
def epoch_end_compute_and_log_metrics(
    pl: pl.LightningModule, mode: str, metrics: OrderedDict[str, MetricBase]
) -> None:
    """
