...
class MetricEnsemble(MetricDefault):
    def __init__(self, pred_keys: Sequence[str], target: Optional[str] = None, \
                method: Optional[str] = 'average', output_file: Optional[str] = None, \
                output_pred_key: Optional[str] = 'preds', \
                output_target_key: Optional[str] = 'target', **kwargs):
        """
        ...
        """
        ensemble = partial(self._ensemble, method=method, output_file=output_file, \
                            output_pred_key=output_pred_key, output_target_key=output_target_key)
