...
class  MetricApplyThresholds(MetricMultiClassDefault):
    def __init__(self, pred: str, class_names: Optional[Sequence[str]] = None, \
                 operation_point: Union[float, Sequence[Tuple[int, float]], str, None] = None, **kwargs):
        """
        ...
        """
        super().__init__(pred=pred, target=None, extract_ids=True, metric_func=self._apply_thresholds, \
                         class_names=class_names, operation_point=operation_point,**kwargs)

    def _apply_thresholds(self, pred: Sequence[np.ndarray], ids: Sequence[Hashable], operation_point: Union[float, Sequence[Tuple[int, float]], str, None] = None):
