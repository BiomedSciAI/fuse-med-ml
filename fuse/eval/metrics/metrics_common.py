...
class MetricCollector(MetricBase):
    ...
    def __init__(self, pre_collect_process_func: Optional[Callable] = None, post_collect_process_func: Optional[Callable] = None, **keys_to_collect: Dict[str, str]):
        """
...
class MetricWithCollectorBase(MetricBase):
    ...
    """
    def __init__(self, pre_collect_process_func: Optional[Callable] = None, post_collect_process_func: Optional[Callable] = None, external_data_collector: Optional[MetricCollector] = None, extract_ids: bool = False, **kwargs) -> None:
        """
        ...
        self._keys_from_results = {n:k[len("results:"):] for n, k in kwargs.items() if isinstance(k, str) and k.startswith("results:")}
        self._value_args = {n:k for n, k in kwargs.items() if k is not None and not isinstance(k, str)}
        self._collector = MetricCollector(pre_collect_process_func, post_collect_process_func, **self._keys_to_collect) if external_data_collector is None else external_data_collector
...
class GroupAnalysis(MetricWithCollectorBase):
    ...
    def eval(self, results: Dict[str, Any] = None, ids: Optional[Sequence[Hashable]] = None) -> Dict[str, Any]:
        ...
        if not ids:
            raise Exception("Error: group analysis is supported only when a unique identifier is specified. Add key 'id' to your data") 
        ids = np.array(ids)
...
class CI(MetricWithCollectorBase):
    ...
    def __init__(self, 
        ...
        # verify
        assert ci_method in ["NORMAL", "PERCENTILE", "PIVOTAL"], f"Error: unexpected confidence interval method: {ci_method}"
    ...
    def eval(self, results: Dict[str, Any] = None, ids: Optional[Sequence[Hashable]] = None) -> Dict[str, Any]:
        ...
        if ids is None:
            ids = self._collector.get_ids()
        if not ids:
            raise Exception("Error: confidence interval is supported only when a unique identifier is specified. Add key 'id' to your data") 
        ids = np.array(ids)
        ...
        if isinstance(original_sample_results, dict):
            ...
            ci_results = self._compute_stats(self._ci_method, original_sample_results, boot_results, self._conf_interval)

        return ci_results
    
    @staticmethod
    def _compute_stats(ci_method: str, orig: Union[float, np.ndarray], samples: Sequence[Union[float, np.ndarray]], confidence: float) -> Dict[str, Union[np.ndarray, float]]:
        ...
        return {'org': orig, 'mean': np.mean(samples), 'std': np.std(samples),
                'conf_interval': confidence, 'conf_lower': confidence_lower, 'conf_upper': confidence_upper}


    @staticmethod
    def compute_confidence_interval(method: str, org_statistic: Union[float, np.ndarray], bootstrap_statistics: Sequence[Union[float, np.ndarray]], confidence: float) -> Tuple[float, float]:
