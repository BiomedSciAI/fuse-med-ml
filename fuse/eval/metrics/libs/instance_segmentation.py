...
class MetricsInstanceSegmentaion:
    ...
    def convert_to_COCO_type(input_list : Sequence[np.ndarray],height : int  ,width: int, segmentation_type : str )-> Dict:
        ...
        if segmentation_type == 'uncompressed_RLE':
            output_list = [MetricsInstanceSegmentaion.convert_uncompressed_RLE_COCO_type(element,height,width) for element in input_list]          
        elif segmentation_type == 'polygon':
            output_list = [MetricsInstanceSegmentaion.convert_polygon_COCO_type(element,height,width) for element in input_list] 
    ...
    @staticmethod
    def iou_jaccard(pred : Sequence[np.ndarray] , target :  Sequence[np.ndarray] , segmentation_pred_type: str,segmentation_target_type: str, height : int , width : int ) -> np.ndarray:
        """
