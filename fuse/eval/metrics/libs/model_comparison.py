...
class ModelComparison:
    ...
    @staticmethod
    def bootstrap_margin_superiority(name: Optional[str], test_values: Sequence[float], reference_values: Sequence[float], margin: float = 0.0, margin_type: str = "abs") -> Dict[str, float]:
        """
    ...
    @staticmethod
    def bootstrap_margin_non_inferiority(name: Optional[str], test_values: Sequence[float], reference_values: Sequence[float], margin: float = 0.0, margin_type: str = "abs") -> Dict[str, float]:
        """
        compare method - to be used in PairedBootstrap. 
    ...
    @staticmethod
    def bootstrap_margin_equality(name: Optional[str], test_values: Sequence[float], reference_values: Sequence[float], margin: float = 0.0, margin_type: str = "abs") -> Dict[str, float]:
        """
        compare method - to be used in PairedBootstrap. 
    ...
    @staticmethod
    def delong_auc_test(pred1: Sequence[np.ndarray],
        ...
        # empirical AUC:
        emp_auc_1 = (1./(m*n)) * PSI1.sum()
        emp_auc_2 = (1./(m*n)) * PSI2.sum()

        # structural components (DeLong et al. 1988):
        V10_1 = (1./n) * PSI1.sum(1)
        V10_2 = (1./n) * PSI2.sum(1)

        V01_1 = (1./m) * PSI1.sum(0)
        V01_2 = (1./m) * PSI2.sum(0)

        # covariance matrix
        S10 = (1.0/(m-1)) * np.array([[np.linalg.norm(V10_1-emp_auc_1)**2, (V10_1-emp_auc_1).dot(V10_2-emp_auc_2)],\
                                      [(V10_2-emp_auc_2).dot(V10_1-emp_auc_1), np.linalg.norm(V10_2-emp_auc_2)**2]])

        S01 = (1.0/(n-1)) * np.array([[np.linalg.norm(V01_1-emp_auc_1)**2, (V01_1-emp_auc_1).dot(V01_2-emp_auc_2)],\
        ...
        results = {'p_value': p_value, 'z': z, \
                    'auc1': emp_auc_1, 'auc2': emp_auc_2, \
                    'cov11': S[0,0], 'cov12': S[0,1], \
                    'cov22': S[1,1]}
