        ...
        for label in labels:
            ...
            if gt_empty and pred_empty:
                ...
                if pixel_weight is None or pixel_weight[label] is None:
                    ...
                    scores[label] = 1.0 - distance.dice(mask_pred.flatten(),mask_gt.flatten(),pixel_weight[label].flatten())
        ...
        for label in labels:
            ...
            if gt_empty and pred_empty:
                ...
                if pixel_weight is None or pixel_weight[label] is None:
                    ...
                    (nft, ntf) = MetricsSegmentation.get_tf_ft_values_from_bool_array_with_weights(pred, target, pixel_weight[label])
        ...
        for label in labels:
            ...
            if gt_empty and pred_empty:
                ...
                if pixel_weight is None or pixel_weight[label] is None:
                    ...
                    overlap = np.sum(intersection) / min(np.sum(pred * pixel_weight[label]),np.sum(target * pixel_weight[label]))
                    scores[label] = overlap
        return scores
