...
class Calibration:
    ...
    def reliability_diagram(
        ...
        assert(all([math.isclose(p.sum(), 1, rel_tol=1e-5) for p in pred])) # make sure that the inputs make proper probabilities
        ...
        for i in range(len(conf_vec)-1):
            cnt_correct_in_conf = sum([p_cls == t for (p_cls,p,t) in zip(cls_pred, max_pred, target) if p>=conf_vec[i] and p<conf_vec[i+1]])
        ...
        if output_filename is not None:
            ...
            ax1.set_title(f"Confidence Histogram. Avg. Accuracy: {np.round(avg_acc, 2)}, Avg. Confidence: {np.round(avg_conf, 2)}")
    ...
    @staticmethod
    def ece(pred: Sequence[Union[np.ndarray, float]], target: Sequence[Union[np.ndarray, int]], 
            num_bins: Optional[int] = 10,
            num_quantiles: Optional[int] = None) -> float:
        """
        ...
        """
        reliability_results = Calibration.reliability_diagram(pred, target, num_bins=num_bins, num_quantiles=num_quantiles, output_filename=None)
        ...
        ece = (1./total_samples)* np.sum(num_samples_per_bin*np.abs(accuracy_vec - conf_vec))
