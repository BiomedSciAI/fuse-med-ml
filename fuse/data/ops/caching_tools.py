...
def get_function_call_str(func, *_args, **_kwargs) -> str:
    ...
    args_flat_str += '@' + str(inspect.getmodule(func)) #adding full (including scope) name of the function, for the case of multiple functions with the same name
...
def value_to_string(val:Any, warn_on_types:Optional[Sequence]=None) -> str:
    ...
    if warn_on_types is not None:
        if isinstance(val, tuple(list(warn_on_types))):
            warnings.warn(f'type {type(val)} is possibly participating in hashing, this is usually not optimal performance wise.')
    ans = str(val)
    if ans.startswith('<'):
        return ''
    return str(val)
