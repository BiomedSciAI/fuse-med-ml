...
class TestVisProbe(unittest.TestCase):
    def test_basic_show(self):
        ...
        sample_dict = NDict(
            {"sample_id": "a", "data": {"test_pipeline": {testing_img_key: 6}}}
        )
        ...
        self.assertEqual(
            DebugVisualizer.acc[0][0][f"groups.group0.{g1_testing_key}.value"], 5
        )
        self.assertEqual(
            DebugVisualizer.acc[1][0][f"groups.group0.{g1_testing_key}.value"], 6
        )
    ...
    def test_multi_label(self):
        ...
        self.assertEqual(
            DebugVisualizer.acc[0][0][f"groups.group0.{test_image_key}.value"], 5
        )
        self.assertEqual(
            DebugVisualizer.acc[0][0][f"groups.group0.{test_seg_key}.value"], 4
        )
        self.assertFalse("group1" in DebugVisualizer.acc[0][0]["groups"])
        self.assertEqual(
            DebugVisualizer.acc[1][0][f"groups.group0.{test_image_key}.value"], 6
        )
        self.assertEqual(
            DebugVisualizer.acc[1][0][f"groups.group0.{test_seg_key}.value"], 4
        )
    ...
    def test_multi_groups(self):
        ...
        self.assertEqual(
            DebugVisualizer.acc[0][0][f"groups.group0.{test_image_key_g1}.value"], 5
        )
        self.assertEqual(
            DebugVisualizer.acc[0][0][f"groups.group1.{test_image_key_g2}.value"], 4
        )
        self.assertEqual(
            DebugVisualizer.acc[1][0][f"groups.group0.{test_image_key_g1}.value"], 6
        )
        self.assertEqual(
            DebugVisualizer.acc[1][0][f"groups.group1.{test_image_key_g2}.value"], 4
        )
        DebugVisualizer.acc.clear()
    ...
    def test_collected_show(self):
        ...
        self.assertEqual(
            DebugVisualizer.acc[0][1][f"groups.group0.{test_image_key}.value"], 6
        )
        DebugVisualizer.acc.clear()
    ...
    def test_reverse_compare(self):
        ...
        """
        revfor_flags = (
            VisFlag.FORWARD | VisFlag.ONLINE | VisFlag.REVERSE | VisFlag.SHOW_COLLECTED
        )
        ...
        sample_dict = NDict(
            {"sample_id": "a", "data": {"test_pipeline": {testing_img_key: 4}}}
        )

        sample_dict = pipe(sample_dict)
        self.assertEqual(sample_dict[g1_testing_image_key], 7)
        sample_dict = pipe.reverse(
            sample_dict, g1_testing_image_key, g1_testing_image_key
        )
        self.assertEqual(len(DebugVisualizer.acc), 1)
        self.assertEqual(len(DebugVisualizer.acc[0]), 2)
        test_image_key = g1_testing_image_key.replace(".", "_")
        ...
        self.assertEqual(
            DebugVisualizer.acc[0][1][f"groups.group0.{test_image_key}.value"], 5
        )
    ...
    def test_multiple_reverse(self):
        ...
        test_image_key = g1_testing_image_key.replace(".", "_")
        self.assertEqual(
            DebugVisualizer.acc[0][0][f"groups.group0.{test_image_key}.value"], 6
        )
        self.assertEqual(
            DebugVisualizer.acc[0][1][f"groups.group0.{test_image_key}.value"], 6
        )
        self.assertEqual(
            DebugVisualizer.acc[1][0][f"groups.group0.{test_image_key}.value"], 5
        )
        self.assertEqual(
            DebugVisualizer.acc[1][1][f"groups.group0.{test_image_key}.value"], 5
        )
        self.assertEqual(sample_dict[g1_testing_image_key], 4)
