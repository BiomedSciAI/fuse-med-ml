...
from fuse.data.key_types_for_testing import (
    DataTypeForTesting,
...
class TestOpBase(unittest.TestCase):
    def test_for_type_detector(self) -> None:
        ...
        self.assertEqual(
            td.get_type(sample, "data.cc.img_for_testing"),
            DataTypeForTesting.IMAGE_FOR_TESTING,
        )
        self.assertEqual(
            td.get_type(sample, "data.cc_img_for_testing"),
            DataTypeForTesting.IMAGE_FOR_TESTING,
        )
        self.assertEqual(
            td.get_type(sample, "data.img_seg_for_testing"),
            DataTypeForTesting.SEG_FOR_TESTING,
        )
        self.assertEqual(
            td.get_type(sample, "data.imgseg_for_testing"),
            DataTypeForTesting.SEG_FOR_TESTING,
        )
        self.assertEqual(td.get_type(sample, "data"), DataTypeBasic.UNKNOWN)
        self.assertEqual(
            td.get_type(sample, "bbox_for_testing"), DataTypeForTesting.BBOX_FOR_TESTING
        )
        self.assertEqual(
            td.get_type(sample, "a.bbox_for_testing"),
            DataTypeForTesting.BBOX_FOR_TESTING,
        )
