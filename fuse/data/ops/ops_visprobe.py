...
class VisProbe(OpBase):
    ...
    def _extract_data(self, sample_dict: NDict, keys, op_id):
        if type(keys) is list:
            ...
            keys = {
                f"group{i}": keys[i : i + keys_per_group]
                for i in range(0, len(keys), keys_per_group)
            }
    ...
    def _handle_flags(self, flow, sample_dict: NDict, op_id: Optional[str]):
        ...
        if (
            VisFlag.SHOW_ALL_COLLECTED in self._flags
            or VisFlag.SHOW_COLLECTED in self._flags
        ) and ((both_fr and dir_reverse) or not both_fr):
