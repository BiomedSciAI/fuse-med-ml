...
class SamplesCacher:    
        ...
        for d in dirs_to_check:
            ...
            for found_dir in found_sub_dirs:
                ...
                if os.path.basename(found_dir) != self._pipeline_desc_hash:
                    raise Exception(f'Found samples cache for pipeline hash {os.path.basename(found_dir)} which is different from the current loaded pipeline hash {self._pipeline_desc_hash} !!\n'
    ...
    def delete_cache(self) -> None:
        ...
        dirs_to_delete = [os.path.realpath(os.path.join(x, '..')) for x in dirs_to_delete] #one dir above the pipeline hash dir
    ...
    def get_orig_sample_id_hash(orig_sample_id):
        ...
        if orig_sample_id_str.startswith('<') and orig_sample_id_str.endswith('>'): #and '0x' in orig_sample_id_str
            ...
            raise Exception(f'You must implement a proper __str__ for orig_sample_id. String representations like <__main__.SomeClass at 0x7fc3e6645e20> are not descriptibe enough and also not persistent between runs. Got: {orig_sample_id_str}')
...
def _get_available_write_location(cache_dirs:List[str], max_allowed_used_space=0.95):               
    ...
    raise Exception('Could not find any location to write.\n'
