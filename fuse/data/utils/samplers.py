    ...
    def __init__(
        ...
        **dataset_get_multi_kwargs,
        ...
        self._balanced_class_indices = [
            np.where(self._balanced_classes == cls_i)[0]
            for cls_i in range(self._num_balanced_classes)
        ]
        self._balanced_class_sizes = [
            len(self._balanced_class_indices[cls_i])
            for cls_i in range(self._num_balanced_classes)
        ]
        ...
        if self._num_batches is None:
            if self._mode == "exact":
                ...
                # approximate size!
                samples_per_batch = [
                    val * self._batch_size for val in self._balanced_class_weights
                ]
    ...
    def _make_batch(self) -> list:
        ...
        if self._mode == "exact":
            ...
            batch_index_to_class = np.random.choice(
                np.arange(self._num_balanced_classes),
                self._batch_size,
                p=self._balanced_class_weights,
            )
