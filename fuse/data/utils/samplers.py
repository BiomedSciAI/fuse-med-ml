    ...
    def __init__(
        ...
        **dataset_get_multi_kwargs,
        ...
        if isinstance(balanced_class_weights, Sequence):
            self._balanced_class_weights = {i: w for i, w in enumerate(balanced_class_weights)}
        else:
        ...
        self._balanced_class_indices = {
            cls_i: np.where(self._balanced_classes == cls_i)[0] for cls_i in self._balanced_class_values
        }
        self._balanced_class_sizes = {
            cls_i: len(self._balanced_class_indices[cls_i]) for cls_i in self._balanced_class_values
        }
        ...
        if self._num_batches is None or self._num_batches in ["over_sample", "down_sample"]:
            if self._mode == "exact":
                ...
                # approximate size!
                samples_per_batch = {
                    cls_i: val * self._batch_size for cls_i, val in self._balanced_class_weights.items()
                }
    ...
    def _make_batch(self) -> list:
        ...
        if self._mode == "exact":
            ...
            batch_index_to_class = np.random.choice(
                self._balanced_class_values, self._batch_size, p=self._balanced_class_weights_list
            )
