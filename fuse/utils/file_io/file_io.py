...
def load_hdf5(filename:str, 
    ...
    for k in h5f.keys():
        ...
        if index is None:
            ...
            assert len(index)>0
            assert isinstance(index[0], (slice,type(Ellipsis)))  #checking just the first, but it should be all of them       
...
def remove_dir_content(dir_path: str, ignore_files: Iterable[str] = tuple(), force_reset: bool = False) -> None:
    ...
    if num_files > 0:
        ...
        force_remove = True if force_reset else Misc.query_yes_no(
