...
class FuseMetricMultiClassBSS(FuseMetricBase):
    ...
    def process(self) -> float:
        ...
        result = multi_class_bss(np.array(self.epoch_preds), np.array(self.epoch_targets, dtype=np.int))
        return result
...
def multi_class_bss(predictions: np.ndarray, targets: np.ndarray) -> float:
    ...
    # no skill BS
    no_skill_prediction = [(targets == target_cls).sum() / targets.shape[0] for target_cls in range(predictions.shape[-1])]
