...
class OpRotation3D(OpBase):
    ...
    def __call__(
        ...
        aug_tensor = rotation_in_3d(
            aug_input, ax1_rot=ax1_rot, ax2_rot=ax2_rot, ax3_rot=ax3_rot
        )
...
def rotation_in_3d(
    aug_input: Tensor, ax1_rot: float = 0.0, ax2_rot: float = 0.0, ax3_rot: float = 0
):
    """
    ...
    if ax1_rot != 0:
        ...
        aug_input = geometry.unsqueeze_3D_from_2D(
            rot_squeeze, axis_squeeze=1, channels=channels
        )
    if ax2_rot != 0:
        ...
        aug_input = geometry.unsqueeze_3D_from_2D(
            rot_squeeze, axis_squeeze=2, channels=channels
        )
    if ax3_rot != 0:
        ...
        aug_input = geometry.unsqueeze_3D_from_2D(
            rot_squeeze, axis_squeeze=3, channels=channels
        )
