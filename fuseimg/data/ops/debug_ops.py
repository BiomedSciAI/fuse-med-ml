...
def draw_grid_3d_op(input_tensor, start_slice=0, end_slice=None,  line_color=255, thickness=10, type_=cv2.LINE_4, pxstep=50):
    '''
    ...
    for s in range(start_slice, end_slice+1):
        ...
        while x < input_tensor.shape[1]:
            cv2.line(input_tensor[...,s], (x, 0), (x, input_tensor.shape[0]), color=line_color, lineType=type_,
                     thickness=thickness)
            x += pxstep

        while y < input_tensor.shape[0]:
            cv2.line(input_tensor[...,s], (0, y), (input_tensor.shape[1], y), color=line_color, lineType=type_,
                     thickness=thickness)
            y += pxstep
