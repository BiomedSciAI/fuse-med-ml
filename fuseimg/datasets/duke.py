...
class Duke:
    ...
    def dataset(
        ...
        dynamic_pipeline = Duke.dynamic_pipeline(
            data_dir=data_dir, train=train, label_type=label_type, verbose=verbose
        )
    ...
    def static_pipeline(
        ...
        data_dir = (
            Duke.get_data_dir_from_environment_variable()
            if data_dir is None
            else data_dir
        )
        ...
        static_pipeline_steps = [
            ...
            (
                ops_mri.OpResampleStkVolsBasedRef(
                    reference_inx=0, interpolation="bspline"
                ),
        ...
        static_pipeline_steps += [
            (
                ops_read.OpReadDataframe(
                    data=get_duke_raw_annotations_df(data_dir), key_column="Patient ID"
                ),
        ...
        if output_patch_volumes:
            if duke_patch_annotations_df is not None:
                ...
                static_pipeline_steps += [
                    (
                        ops_read.OpReadDataframe(
                            data=duke_patch_annotations_df, key_column="Patient ID"
                        ),
            ...
            static_pipeline_steps += [
                ...
                (
                    ops_mri.OpAddMaskFromBoundingBoxAsLastChannel(
        ...
        static_pipeline = PipelineDefault(
            "static", static_pipeline_steps, verbose=verbose
        )
    ...
    def dynamic_pipeline(
        ...
        volume_key = (
            "data.input.patch_volume"
            if use_entire_lesion_volume
            else "data.input.patch_volume_orig"
        )
        ...
        if train:
            ...
            dynamic_steps += [
                ...
                # ],
                (
                    geometry.OpAugSqueeze3Dto2D(),
                    dict(key="data.input.patch_volume", axis_squeeze=1),
                ),
        ...
        if add_clinical_features or (label_type is not None):
            ...
            dynamic_steps += [
                (
                    ops_read.OpReadDataframe(
                        data=get_duke_clinical_data_df(data_dir),
            ...
            if label_type is not None:
                ...
                dynamic_steps.append(
                    (
                        ops_common.OpLambda(
                            func=partial(
                                remove_entries_with_nan_label, key=key_ground_truth
                            )
                        ),
                        dict(key=None),
                    )
            ...
            if add_clinical_features:
                ...
                dynamic_steps.append(
                    (
                        ...
                        dict(
                            key="data.input.clinical_data",
                            key_out=key_clinical_features,
                        ),
                    )
                )
        ...
        dynamic_steps.append(
            (ops_common.OpKeepKeypaths(), dict(keep_keypaths=keys_2_keep))
        )
...
class OpAddDukeLabelAndClinicalFeatures(OpBase):
    ...
    def __init__(
        self,
        label_type: DukeLabelType,
        is_concat_features_to_input: Optional[bool] = False,
        **kwargs,
    ):
        super().__init__(**kwargs)
    ...
    def __call__(
        ...
        sample_dict[
            key_out_clinical_features
        ] = clinical_features_to_use  # 'data.clinical_features'
...
def get_duke_clinical_data_df(duke_data_dir):
    ...
    annotations_df = pd.read_excel(
        annotations_path, sheet_name="Data", skiprows=3, header=None, names=columns
    )
    return annotations_df


def get_samples_for_debug(data_dir, n_pos, n_neg, label_type, sample_ids=None):
    annotations_df = get_duke_clinical_data_df(data_dir).set_index(
    ...
    for label_val, n_vals in zip([True, False], [n_pos, n_neg]):
        debug_sample_ids += patient_ids[label_values == label_val].values.tolist()[
            :n_vals
        ]
...
def get_series_desc_2_sequence_mapping(metadata_path: str):
    ...
    for i_phase in range(1, 5):
        ...
        for series_desc in series_description_list:
            ...
            if has_match:
                series_desc2 = series_desc.replace(
                    f"{i_phase}ax", f"{i_phase}/ax"
                ).replace(f"{i_phase}Ax", f"{i_phase}/Ax")
...
class DukeRadiomics(Duke):
    ...
    def dynamic_pipeline(
        ...
        dynamic_steps = [
            ...
            (
                ops_mri.OpExtractRadiomics(
                    radiomics_extractor, radiomics_extractor_setting
                ),
                dict(
                    key_in_vol_4d="data.input.volume4D",
                    key_out_radiomics_results="data.radiomics",
                ),
            ),
        ]
    ...
    def dataset(
        ...
        dynamic_pipeline = DukeRadiomics.dynamic_pipeline(
            radiomics_extractor_setting=radiomics_extractor_setting,
