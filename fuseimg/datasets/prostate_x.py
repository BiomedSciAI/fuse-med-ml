    ...
    def static_pipeline(
        ...
        static_pipeline_steps = [
            ...
            (
                ...
                dict(
                    key_path_out="data.input.mri_path",
                    key_patient_id_out="data.input.patient_id",
                ),
            ),
            ...
            (
                ops_mri.OpReadSTKImage(
                    seq_id="ktrans",
                    get_image_file=partial(
                        get_ktrans_image_file_from_sample_id, data_dir=root_path
                    ),
                ),
                dict(
                    key_sequence_prefix="data.input.sequence.",
        ...
        static_pipeline_steps += [
            ...
            (
                ops_mri.OpCreatePatchVolumes(
                    lsn_shape=(
                        ProstateX.PATCH_Z_SIZE,
                        ProstateX.PATCH_XY_SIZE,
                        ProstateX.PATCH_XY_SIZE,
                    ),
                    name_suffix="_T0",
                    pos_key="pos",
...
def fix_certain_b_sequences(sample_dict, key_in_volumes_info, key_volumes):
    ...
    has_b_to_fix = np.any(
        [
            (info["seq_id"] == "b") and (info["series_desc"] in B_SER_FIX)
...
def get_ktrans_image_file_from_sample_id(sample_id, data_dir):
    ...
    ktrans_patient_dir = os.path.join(
        data_dir, "ProstateXKtrains-train-fixed", patient_id
    )
...
class OpAdProstateXLabelAndClinicalFeatures(OpBase):
    ...
    def __call__(
        ...
        if False:
            ...
            clinical_features_to_use = zone2feature[
                clinical_features[features_to_use[0]]
            ]
...
def get_prostate_x_annotations_df(data_dir):
    if True:
        ...
        annotations_df = pd.read_csv(
            os.path.join(data_dir, "Lesion Information", "ProstateX-Findings-Train.csv")
        )
    ...
    annotations_df["Sample ID"] = (
        annotations_df["Patient ID"] + "_" + annotations_df["fid"].astype(str)
    )
